...
public class GPedidos1 extends javax.swing.JPanel {
    ...
    private void initComponents() {
        ...
        jBGuardar.setIcon(new javax.swing.ImageIcon(getClass().getResource("/IMG/pedido_1.png"))); // NOI18N
        ...
        jPanel1Layout.setVerticalGroup(
            ...
            .addGroup(jPanel1Layout.createSequentialGroup()
                ...
                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                .addGroup(jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    .addComponent(jSIDMesero, javax.swing.GroupLayout.Alignment.TRAILING, javax.swing.GroupLayout.PREFERRED_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.PREFERRED_SIZE)
        ...
        CANT.addPropertyChangeListener(new java.beans.PropertyChangeListener() {
            public void propertyChange(java.beans.PropertyChangeEvent evt) {
                CANTPropertyChange(evt);
            }
        });

        IDPROD.addChangeListener(new javax.swing.event.ChangeListener() {
            public void stateChanged(javax.swing.event.ChangeEvent evt) {
                IDPRODStateChanged(evt);
            }
        });
    ...
    private void jDate2PropertyChange(java.beans.PropertyChangeEvent evt) {//GEN-FIRST:event_jDate2PropertyChange
        if ("date".equals(evt.getPropertyName())) {
            // Verifica que ambas fechas estén seleccionadas antes de continuar
            if (jDate1.getDate() != null && jDate2.getDate() != null) {
                // Obtiene las fechas de jDate1 y jDate2
                Date inicioDate = jDate1.getDate();
                Date finDate = jDate2.getDate();

                // Convierte las fechas a LocalDateTime
                LocalDateTime fechaInicio = inicioDate.toInstant().atZone(ZoneId.systemDefault()).toLocalDate().atStartOfDay();
                LocalDateTime fechaFin = finDate.toInstant().atZone(ZoneId.systemDefault()).toLocalDate().atTime(LocalTime.MAX);

                // Llama al método de filtrado con los parámetros obtenidos
                filtrarPorRangoFechaHora(fechaInicio, fechaFin);
            }
        }
    }//GEN-LAST:event_jDate2PropertyChange

    private void IDPRODStateChanged(javax.swing.event.ChangeEvent evt) {//GEN-FIRST:event_IDPRODStateChanged
        int idProducto = (int) IDPROD.getValue();

        // Verifica que el idProducto no sea 0 o un valor no válido antes de realizar la búsqueda
        if (idProducto > 0) {
            Producto producto = productoData.buscarProductoPorId(idProducto);

            if (producto != null) {
                // Actualiza el JComboBox con el nombre del producto encontrado
                jCPROD.setSelectedItem(producto.getNombre());
            } else {
                // Si el producto no se encuentra, limpia la selección del JComboBox
                jCPROD.setSelectedIndex(-1);
                JOptionPane.showMessageDialog(this, "No se encontró un producto con el ID especificado.", "Error", JOptionPane.WARNING_MESSAGE);
            }
        } else {
            // Si IDPROD es 0, limpia la selección del JComboBox
            jCPROD.setSelectedIndex(-1);
        }
    ...
    private void cargarPedidoEnComponentes(int idPedido) {
        ...
        if (pedido != null) {
            ...
            // Mostrar un mensaje de advertencia si no se encuentra el pedido
    ...
    private void filtrarPorRangoFechaHora(LocalDateTime fechaInicio, LocalDateTime fechaFin) {
        ...
        if (fechaInicio.isAfter(fechaFin)) {
            JOptionPane.showMessageDialog(this, "La fecha de inicio debe ser menor o igual a la fecha de fin.", "Error en el rango de fechas", JOptionPane.WARNING_MESSAGE);
            return;
        }
        ...
        jTProd.getSelectionModel().addListSelectionListener(e -> {
            if (!e.getValueIsAdjusting()) { // Verifica que la selección haya terminado
                ...
                if (selectedRow != -1) {
                    ...
                    double precioUnitario = (double) modeloProductos.getValueAt(selectedRow, 4) / cantidad;
                    ...
                    // Configurar el precio total en `jTPRECIO` multiplicando por la cantidad
                    jTPRECIO.setText(String.valueOf(precioUnitario * cantidad));
                    jTPRECIO.setEditable(false);
                    ...
                    // Añadir listener al spinner de cantidad para actualizar el precio
                    CANT.addChangeListener(ev -> {
                        int nuevaCantidad = (int) CANT.getValue();
                        jTPRECIO.setText(String.valueOf(precioUnitario * nuevaCantidad));
                    });
                }
            }
        });
    ...
    private void configureDateTimePickers() {
        ...
        jDate1.addPropertyChangeListener("date", evt -> {
            if (jDate1.getDate() != null && jDate2.getDate() != null) {
                LocalDateTime inicio = jDate1.getDate().toInstant().atZone(ZoneId.systemDefault()).toLocalDateTime();
                LocalDateTime fin = jDate2.getDate().toInstant().atZone(ZoneId.systemDefault()).toLocalDateTime();
                filtrarPorRangoFechaHora(inicio, fin);
            }
        });

        jDate2.addPropertyChangeListener("date", evt -> {
            if (jDate1.getDate() != null && jDate2.getDate() != null) {
                LocalDateTime inicio = jDate1.getDate().toInstant().atZone(ZoneId.systemDefault()).toLocalDateTime();
                LocalDateTime fin = jDate2.getDate().toInstant().atZone(ZoneId.systemDefault()).toLocalDateTime();
                filtrarPorRangoFechaHora(inicio, fin);
